...
# --- App Configuration ---
st.set_page_config(layout="wide", page_title="Factory Layout Optimizer | Cost Analysis")
...
[data-testid="stExpander"] summary {
    background-color: #E6E6E6; /* Slightly darker light gray */
...
[data-testid="stMetric"] div[data-testid="stMetricDelta"] {
    color: #28a745; /* Dark Green for positive delta */
}
...
st.title("🏭 GA Factory Layout & A* Flow Optimizer | Cost Analysis")
st.info(
    "The GA finds the optimal layout by **minimizing cost factors** (MHS distance, wasted time, unused area) while **maximizing throughput**."
)
...
def calculate_layout_cost(total_distance, utilization_ratio, max_stage_time_s, factory_w, factory_h, cost_params):
    # Cost 1: Material Handling Cost (MHS) based on total distance
    mhs_cost = total_distance * cost_params['cost_per_unit_distance']

    # Cost 2: WIP/Idle Time Cost (Cost of the bottleneck)
    # Annual WIP/Idle cost assumes 8 hours/day, 250 days/year
    annual_operating_seconds = 8 * 3600 * 250 
    
    # Cost incurred for every unit *processed* by the bottleneck (max_stage_time)
    cost_per_unit_wip_time = max_stage_time_s * cost_params['cost_per_sec_bottleneck']
    
    # Total annual WIP/Idle cost is proportional to the bottleneck time
    wip_idle_cost = cost_per_unit_wip_time * SECONDS_PER_HOUR * annual_operating_seconds / max_stage_time_s

    # Cost 3: Unused Area Cost
    total_area = factory_w * factory_h
    unused_area = total_area * (1 - utilization_ratio)
    unused_area_cost = unused_area * cost_params['cost_per_unused_area']
    
    total_estimated_cost = mhs_cost + wip_idle_cost + unused_area_cost
    
    return total_estimated_cost, mhs_cost, wip_idle_cost, unused_area_cost
...
def calculate_fitness(chromosome, machines_defs_ordered_by_proc_seq, process_seq_ids,
                      ...
                      all_machines_data, fitness_weights, constraint_params, cost_params):
    ...
    # 5. Cost Calculation (The main optimization target)
    ...
    fitness_val = revenue_value - total_estimated_cost - total_penalties_cost
...
def visualize_layout_plt(grid_layout_to_show, machine_positions_map, factory_w, factory_h, 
    ...
    plt.title("Optimized Factory Layout (GA) with Zone Constraint", fontsize=12, color='#333333')
    plt.xlabel("Factory Width (X)", color='#333333')
    plt.ylabel("Factory Height (Y)", color='#333333')
...
def visualize_layout_with_paths(layout_data, all_paths, flow_density_grid):
    ...
    ax_path.set_facecolor('#FFFFFF')
    ax_path.tick_params(colors='#333333')
    ax_path.xaxis.label.set_color('#333333')
    ax_path.yaxis.label.set_color('#333333')
    ax_path.title.set_color('#333333')
    ax_path.spines['bottom'].set_color('#333333')
    ax_path.spines['top'].set_color('#333333')
    ax_path.spines['left'].set_color('#333333')
    ax_path.spines['right'].set_color('#333333')
    ...
    ax_path.grid(True, linestyle='--', alpha=0.5, color='#BBBBBB')
    ...
    ax_path.set_title("Factory Layout with Optimized Continuous Paths", fontsize=14, color='#333333')
    ax_path.set_xlabel("Factory Width (X)", color='#333333')
    ax_path.set_ylabel("Factory Height (Y)", color='#333333')
    ...
    ax_heat.tick_params(colors='#333333')
    ax_heat.xaxis.label.set_color('#333333')
    ax_heat.yaxis.label.set_color('#333333')
    ax_heat.title.set_color('#333333')
    ax_heat.spines['bottom'].set_color('#333333')
    ax_heat.spines['top'].set_color('#333333')
    ax_heat.spines['left'].set_color('#333333')
    ax_heat.spines['right'].set_color('#333333')
    ...
    ax_heat.grid(True, linestyle='--', alpha=0.5, color='#BBBBBB')
    ...
    # Draw Heatmap
    if max_flow > 0:
        # Use red/orange for high flow density
        ...
        cbar.set_label('Material Flow Density (Path Overlaps)', color='#333333')
        plt.setp(plt.getp(cbar.ax.yaxis, 'ticklabels'), color='#333333')
    ...
    ax_heat.set_title("Factory Congestion Heatmap (A* Flow Analysis)", fontsize=14, color='#333333')
    ax_heat.set_xlabel("Factory Width (X)", color='#333333')
    ax_heat.set_ylabel("Factory Height (Y)", color='#333333')
    ax_heat.invert_yaxis() 
...
def generate_performance_summary(ga_metrics, a_star_metrics, process_sequence, machines_defs, target_tph, cost_params, current_total_distance):
    ...
    # Cost metrics calculated inside calculate_fitness are available in ga_metrics["total_cost"]
    
    # 1. Calculate Baseline Distance Cost
    baseline_mhs_cost = REFERENCE_BASELINE_DISTANCE * cost_params['cost_per_unit_distance']
    
    # 2. Calculate Optimized MHS Cost
    optimized_mhs_cost = current_total_distance * cost_params['cost_per_unit_distance']
    
    # 3. Calculate Savings
    mhs_savings = baseline_mhs_cost - optimized_mhs_cost
    
    # 4. Total Cost from GA run
    total_estimated_cost = ga_metrics.get("total_cost", 0.0)
    
    print("\n" + "="*80)
    print(f"{'FACTORY LAYOUT OPTIMIZATION SUMMARY & COST ANALYSIS':^80}")
    print("="*80)
    
    print(f"\n{'ECONOMIC ANALYSIS (ROUGH ANNUAL ESTIMATE)':^80}")
    print("-" * 80)
    
    print(f"{'Metric':<35} | {'Value / Cost':<20} | {'Savings / Optimization':<20}")
    print("-" * 80)
    print(f"{'Total Optimized Est. Cost':<35} | ${total_estimated_cost:<19.2f} | {'-':<20}")
    print(f"{'Baseline MHS Cost (Ref. {REFERENCE_BASELINE_DISTANCE:.1f} units)':<35} | ${baseline_mhs_cost:<19.2f} | {'-':<20}")
    print(f"{'MHS Cost Savings (Optimization)':<35} | ${optimized_mhs_cost:<19.2f} | ${mhs_savings:<19.2f} (Savings)")
...
def run_pathfinding_analysis(layout_data, material_travel_speed):
    ...
    with redirect_stdout(f):
        try:
            ...
            for machine_id_str, pos_info in machine_positions_map_str.items():
            ...
            with redirect_stdout(summary_text):
                generate_performance_summary(ga_metrics, a_star_metrics, process_sequence, machines_definitions, layout_data["target_tph"], layout_data["cost_params"], ga_metrics.get("distance", 0.0))
...
def generate_analysis_plots(logs, target_tph, target_util_thresh):
    ...
    # --- Plot Styling for White BG ---
    plot_bg_color = '#F8F8F8'
    text_color = '#333333'
    grid_color = '#BBBBBB'
    
    # Plot 1: Fitness Progress
    axs[0, 0].set_facecolor(plot_bg_color)
    ...
    axs[0, 0].set_title('GA Fitness Progress', color=text_color); axs[0, 0].legend(fontsize=8, facecolor='white'); axs[0, 0].grid(True, linestyle=':', alpha=0.5, color=grid_color)
    
    # Plot 2: Total Distance
    axs[0, 1].set_facecolor(plot_bg_color)
    ...
    axs[0, 1].set_title('Best Individual Distance', color=text_color); axs[0, 1].legend(fontsize=8, facecolor='white'); axs[0, 1].grid(True, linestyle=':', alpha=0.5, color=grid_color)
    if any(d == float('inf') for d in logs['best_distance']): axs[0, 1].text(0.05, 0.95, "Note: 'inf' distances capped", transform=axs[0, 1].transAxes, fontsize=8, verticalalignment='top', color=text_color)

    # Plot 3: Throughput vs. Target
    axs[0, 2].set_facecolor(plot_bg_color)
    ...
    axs[0, 2].set_title('Best Individual Throughput', color=text_color); axs[0, 2].legend(fontsize=8, facecolor='white'); axs[0, 2].grid(True, linestyle=':', alpha=0.5, color=grid_color)

    # Plot 4: Valid Individuals Ratio
    axs[1, 0].set_facecolor(plot_bg_color)
    ...
    axs[1, 0].set_title('Valid Individuals Ratio', color=text_color); axs[1, 0].legend(fontsize=8, facecolor='white'); axs[1, 0].grid(True, linestyle=':', alpha=0.5, color=grid_color)

    # Plot 5: Zone Constraint Penalty
    axs[1, 1].set_facecolor(plot_bg_color)
    ...
    axs[1, 1].set_title('Zone Proximity Penalty', color=text_color); axs[1, 1].legend(fontsize=8, facecolor='white'); axs[1, 1].grid(True, linestyle=':', alpha=0.5, color=grid_color)

    # Plot 6: Area Utilization Ratio
    axs[1, 2].set_facecolor(plot_bg_color)
    ...
    axs[1, 2].set_title('Area Utilization Ratio', color=text_color); axs[1, 2].legend(fontsize=8, facecolor='white'); axs[1, 2].grid(True, linestyle=':', alpha=0.5, color=grid_color)
    
    # Apply white mode styling to all axes
    for ax_row in axs:
        for ax in ax_row:
...
def run_optimization_process(factory_w, factory_h, target_tph, material_travel_speed,
                             ga_params, fitness_weights, constraint_params, cost_params,
                             machines_definitions, process_sequence,
                             progress_placeholder, status_text):
    
    # Set seed for deterministic runs (helps when comparing runs)
    random.seed(42)
    ...
    for generation in range(1, num_generations + 1):
        ...
        for chromo in population:
            ...
            eval_dict = calculate_fitness(
                ...
                machines_definitions, fitness_weights, constraint_params, cost_params
            )
        ...
        if current_gen_best_fitness > best_overall_fitness:
            ...
            best_overall_metrics = copy.deepcopy(current_gen_best_eval_dict)  # Keep the full metrics dict
            status_text.text(
                f"🌟 Generation {generation}: New best cost-fitness! {best_overall_fitness:.2f}"
            )

        progress_placeholder.text(
            f"Generation {generation}/{num_generations} - BestF: {current_gen_best_fitness:.2f}"
        )
    ...
    layout_data = {
        ...
        "cost_params": cost_params # Pass cost parameters
    }
...
# Cost Factors & Revenue
with st.sidebar.expander("💰 Cost & Revenue Factors", expanded=True):
    st.subheader("Cost Components (Unit/Annual)")
    cost_per_unit_distance = st.number_input("Cost per Unit Distance (MHS/Travel)", value=10.00, format="%.2f", help="e.g., Cost per meter moved.")
...
with st.sidebar.expander("⚖️ Fitness Multipliers", expanded=False):
    # Only multipliers that don't overlap with the primary cost drivers
    zone_penalty_multiplier = st.number_input("Zone Penalty Multiplier", value=1000.0, help="High value ensures hard constraints are met.")
    bonus_for_target = st.number_input("Throughput Bonus Factor", value=500.0, help="Bonus added to Fitness if Target TPH is met.")
...
# Machine Data
with st.sidebar.expander("🤖 Machine & Process Data", expanded=False):
...
if run_button:
    ...
    # 3. Bundle parameters
    ga_params = {
        "population_size": population_size, "num_generations": num_generations, "mutation_rate": mutation_rate, "crossover_rate": crossover_rate, "elitism_count": elitism_count, "tournament_size": tournament_size
    }
    fitness_weights = {
        "zone_penalty_multiplier": zone_penalty_multiplier, "bonus_for_target": bonus_for_target
    }
    constraint_params = {
        "zone_1_target_machines": zone_1_target_machines, "zone_1_max_spread_distance": zone_1_max_spread, "area_util_min_threshold": area_util_min
    }
    cost_params = {
        "cost_per_unit_distance": cost_per_unit_distance,
        "cost_per_sec_bottleneck": cost_per_sec_bottleneck,
        "cost_per_unused_area": cost_per_unused_area,
        "revenue_per_tph_factor": revenue_per_tph_factor
    }
